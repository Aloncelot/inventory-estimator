...
const blankLevel = (props = {}) => ({
  ...
  looseMaterials: {
    /* ...any defaults for loose materials... */
  },
  panelNails: {
    /* ...any defaults for panel nails... */
  },
...
const blankEstimateData = () => ({
  ...
  manufactureEstimate: {
    /* ...defaults for manufacture... */
  },
  nailsAndBracing: {
    /* ...defaults for nails/bracing... */
  },
});
...
export function ProjectProvider({ children, initialProjectId = null }) {
  ...
  const createNewProject = useCallback(
    async (name) => {
      ...
      if (!user || !collectionPath || isSaving) {
        // <-- Fix: Added {}
        console.error(
          "Cannot create project: User not logged in, path missing, or already saving."
        );
        return null;
      }
    ...
    [user, getProjectsCollectionPath, isSaving, db, fetchProjectsList]
  ); // db/fetchProjectsList missing
    ...
    async (pId) => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (
            !data.estimateData.levels ||
            data.estimateData.levels.length === 0
          ) {
  ...
  const saveProject = useCallback(
    async (pId = projectId, data = projectData) => {
      if (!user || !pId || !data || isSaving) {
        console.error(
          "Cannot save project: User not logged in or missing data/ID."
        );
        return;
      }
      const path = getProjectPath(pId);
      ...
      try {
        ...
        setProjectsList((list) =>
          list
            .map((p) =>
              p.id === pId
                ? { ...p, updatedAt: saveData.updatedAt.toDate() }
                : p
            )
  ...
  useEffect(() => {
    if (appId) {
      // <-- Fix: check for appId (not default)
