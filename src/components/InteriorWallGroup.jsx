...
import { calcPlates, calcStuds, calcBlocking, calcSheathing, calcHeader, calcPost } from '@/domain/calculators';
import { parseBoardLengthFt } from '@/domain/lib/parsing';
import { isLVL, isVersaColumn } from '@/domain/lib/families';

// Iconos Lucide
import { ChevronDown, ChevronRight, Cross, Trash2, GripVertical, StickyNote } from 'lucide-react';
...
const getFamily = (selLike) => String(selLike?.familyLabel ?? selLike?.familyDisplay ?? '').toLowerCase();
...
/* ────────────────────────────────────────────────────────────────────────
   SUB-COMPONENTE: FILA EXTRA SORTABLE
   ──────────────────────────────────────────────────────────────────────── */
function SortableExtraRow({ ex, gridRows, updateExtra, removeExtra, getNote, toggleOpen, setNote, heightFt, localExtraWaste, handleLocalExtraWasteChange, handleExtraWasteBlur }) {
      ...
      <div style={style} className="ew-grid ew-row">
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: 2 }}>
            ...
            {ex.type === 'Studs' && (
              <>
                <label style={{ flex: 1 }}><span className="ew-subtle">Len (LF)</span>
                ...
                <label style={{ flex: 1 }}><span className="ew-subtle">Spc (in)</span>
                ...
                <label style={{ flex: 1 }}><span className="ew-subtle">Per Loc</span>
                  <select className="ew-select" value={ex.inputs?.studMultiplier || 1} onChange={e => updateExtra(ex.id, { inputs: { ...ex.inputs, studMultiplier: Number(e.target.value) || 1 } })}>
            ...
            {ex.type === 'Extra blocking' && (
              <label style={{ flex: 1 }}><span className="ew-subtle">Rows (#)</span>
                <input className="ew-input" type="number" min={1} value={ex.inputs?.rows || 1} onChange={e => updateExtra(ex.id, { inputs: { ...ex.inputs, rows: Number(e.target.value) || 1 } })} />
              </label>
            )}
          </div>
        </div>
        ...
        <div className="ew-right">
          <input className="ew-input focus-anim" type="number" value={localExtraWaste[ex.id] ?? ex.wastePct ?? 0}
            onChange={(e) => handleLocalExtraWasteChange(ex.id, e)} onBlur={(e) => handleExtraWasteBlur(ex.id, e)} style={{ width: '100%', textAlign: 'right' }} />
        </div>
...
/* ────────────────────────────────────────────────────────────────────────
   COMPONENTE INTERIOR WALL GROUP
   ──────────────────────────────────────────────────────────────────────── */
export default function InteriorWallGroup({ sectionData, onUpdateSection, title = 'Interior walls', onRemove }) {
  const { kind = 'partition', lengthLF = 0, heightFt = 12, studSpacingIn = 16, studMultiplier = 1, waste = {}, sel = {}, notes = {}, extras = [], collapsed = false } = sectionData;
  ...
  }, [lengthLF, heightFt, studSpacingIn]);
  ...
  // FIX: Patrón de useRef para sustituir useEffectEvent
  const onUpdateRef = useRef(onUpdateSection);
  useEffect(() => { onUpdateRef.current = onUpdateSection; }, [onUpdateSection]);

  const handleDragEnd = (event) => {
    const { active, over } = event;
    if (active.id !== over?.id && onUpdateRef.current) {
      const oldIndex = extras.findIndex((x) => x.id === active.id);
      const newIndex = extras.findIndex((x) => x.id === over.id);
      onUpdateRef.current(prev => ({ ...prev, extras: arrayMove(prev.extras, oldIndex, newIndex) }));
    }
  };

  const updateExtra = useCallback((id, patch) => {
    if (onUpdateRef.current) {
      onUpdateRef.current((prev) => ({ ...prev, extras: (prev.extras || []).map((r) => r.id === id ? { ...r, ...patch } : r) }));
    }
  }, []);

  const removeExtra = useCallback((id) => {
    if (onUpdateRef.current) {
      onUpdateRef.current((prev) => ({ ...prev, extras: (prev.extras || []).filter((r) => r.id !== id) }));
    }
  }, []);

  const addExtra = useCallback((type) => {
    const inputs = type === 'Studs' ? { lengthLF: 10, studSpacingIn: 16, studMultiplier: 1 } : type === 'Extra blocking' ? { rows: 1 } : {};
    if (onUpdateRef.current) {
      onUpdateRef.current((prev) => ({ ...prev, extras: [...(prev.extras || []), { id: `x${Date.now()}`, type, item: null, wastePct: 5, inputs }] }));
    }
  }, []);
  ...
  const baseRows = useMemo(() => {
    ...
    if (kind === 'bearing') rows.push({ key: 'blocking', label: 'Blocking', ...calcBlocking({ lengthLF, heightFt, boardLenFt: blockLen, wastePct: waste.blocking || 0, item: getItem(sel.blocking), unit: getUnit(sel.blocking) }) });
    if (kind === 'shear') rows.push({ key: 'sheathing', label: 'Sheathing (4x8)', ...calcSheathing({ lengthLF, heightFt, wastePct: waste.sheathing || 0, item: getItem(sel.sheathing), unit: getUnit(sel.sheathing) || 'sheet' }) });
    return rows;
  }, [lengthLF, heightFt, studSpacingIn, studMultiplier, sel, waste, kind]);
  ...
  const computedExtras = useMemo(() => {
    return (extras || []).map(r => {
      ...
      if (r.type === 'Header') res = calcHeader({ isLVL: isLVL(fam), headerLF: r.inputs?.headerLF || 0, lvlPieces: 0, lvlLength: 0, boardLenFt, wastePct: r.wastePct, item: getItem(r.item) });
      if (r.type === 'Extra blocking') res = calcPlates({ lengthLF: lengthLF * (r.inputs?.rows || 1), boardLenFt, wastePct: r.wastePct, item: getItem(r.item), unit: getUnit(r.item) });
      if (r.type === 'Extra sheathing') res = calcSheathing({ lengthLF, heightFt, wastePct: r.wastePct, item: getItem(r.item), unit: getUnit(r.item) || 'sheet' });
      if (r.type === 'Post') res = calcPost({ isLinearLF: isLVL(fam) || isVersaColumn(fam), pieces: 0, heightFt: heightFt, piecesPerPost: 0, numPosts: 0, wastePct: r.wastePct, item: getItem(r.item) });
  ...
  useEffect(() => {
    const currentStats = { lengthLF, groupSubtotal: Number(groupSubtotal.toFixed(2)) };
    const currentSig = JSON.stringify(currentStats);
    if (currentSig !== lastSentSigRef.current) {
      lastSentSigRef.current = currentSig;
      if (onUpdateRef.current) {
        onUpdateRef.current((prevData) => ({ ...prevData, ...currentStats }));
      }
    }
  }, [lengthLF, groupSubtotal]);

  const getNote = (k) => ({ ...defaultNote, ...(notes || {})[k] });
  const setNote = useCallback((k, patch) => {
    if (onUpdateRef.current) {
      onUpdateRef.current((prev) => ({ ...prev, notes: { ...prev.notes, [k]: { ...defaultNote, ...prev.notes?.[k], ...patch } } }));
    }
  }, []);
  ...
  return (
    <div className="ew-card">
      <AccordionSection
        open={!collapsed}
        onOpenChange={(isOpen) => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, collapsed: !isOpen }))}
        bar={({ open, toggle }) => (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
            ...
            <EditableTitle value={title} onChange={(n) => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, name: n }))} textClass="text-section-header" />
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '16px', minHeight: '60px', alignItems: 'center' }}>
          {['lengthLF', 'heightFt', 'studSpacingIn'].map((f, i) => (
            <label key={f} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <input className="ew-input focus-anim" type="number" style={{ fontSize: '0.8rem', height: '28px' }} value={[inputValueLF, inputValueHeight, inputValueSpacing][i]} onChange={(e) => [setInputValueLF, setInputValueHeight, setInputValueSpacing][i](e.target.value)} onBlur={(e) => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, [f]: Number(e.target.value) || 0 }))} />
            </label>
          ))}
          <label style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <span className="ew-subtle" style={{ fontSize: '0.7rem' }}>Studs per location</span>
            <select className="ew-select focus-anim" style={{ fontSize: '0.8rem', height: '28px', padding: '0 24px 0 8px' }} value={studMultiplier} onChange={e => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, studMultiplier: Number(e.target.value) || 1 }))}>
              <option value={1}>Single</option><option value={2}>Double</option><option value={3}>Triple</option><option value={4}>Quad</option>
            </select>
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <span className="ew-subtle" style={{ fontSize: '0.7rem' }}>Wall kind</span>
            <select className="ew-select focus-anim" style={{ fontSize: '0.8rem', height: '28px', padding: '0 24px 0 8px' }} value={kind} onChange={e => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, kind: e.target.value }))}>
              <option value="partition">Partition</option><option value="bearing">Bearing</option><option value="shear">Shear</option><option value="knee">Knee</option>
            </select>
          </label>
        </div>
            ...
            return (
              <Fragment key={row.key}>
                <div className="ew-grid ew-row" style={{ display: 'grid', gridTemplateColumns: gridRows, alignItems: 'center' }}>
                  ...
                  <ItemPicker compact onSelect={(i) => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, sel: { ...p.sel, [row.key]: i } }))} value={sel[row.key]} />
                  <div className="ew-right">{Math.ceil(row.qtyRaw || 0)}</div>
                  <div className="ew-right">
                    <input className="ew-input focus-anim" type="number" style={{ width: '100%', textAlign: 'right' }} value={localWaste[row.key] ?? 0} onChange={(e) => setLocalWaste(v => ({ ...v, [row.key]: e.target.value }))} onBlur={(e) => onUpdateRef.current && onUpdateRef.current(p => ({ ...p, waste: { ...p.waste, [row.key]: Number(e.target.value) || 0 } }))} />
                  </div>
                  <div className="ew-right" style={{ fontWeight: 600 }}>{row.qtyFinal || 0}</div>
                  <div className="ew-right" style={{ opacity: 0.7 }}>{row.unit || 'pcs'}</div>
                  <div className="ew-right ew-money">{fmt(row.unitPrice)}</div>
                  <div className="ew-right ew-money" style={{ fontWeight: 700, color: 'var(--turq-200)' }}>{fmt(row.subtotal)}</div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', paddingRight: '8px' }}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'flex-end' }}>
                      <span className="ew-chip" style={{ fontSize: '0.7rem' }}>{n.plan || '—'}</span>
                      <button className="row-action-btn" onClick={() => toggleOpen(noteKey)}><StickyNote size={16} className="gradient-icon" /></button>
          ...
          <div className="ew-rows">
            <SortableContext items={extras.map(x => x.id)} strategy={verticalListSortingStrategy}>
              {computedExtras.map((ex) => (
                <SortableExtraRow key={ex.id} ex={ex} gridRows={gridRows} updateExtra={updateExtra} removeExtra={removeExtra} getNote={getNote} toggleOpen={toggleOpen} setNote={setNote} heightFt={heightFt} localExtraWaste={localExtraWaste}
                  handleLocalExtraWasteChange={(id, e) => setLocalExtraWaste(v => ({ ...v, [id]: e.target.value }))} handleExtraWasteBlur={(id, e) => updateExtra(id, { wastePct: Number(e.target.value) || 0 })} />
              ))}
            </SortableContext>
          </div>
        </DndContext>
        ...
        <div className="ew-footer">
          {['Header', 'Studs', 'Extra blocking'].map(t => (
            <button key={t} className="ew-btn" onClick={() => addExtra(t)}>
              <Cross size={14} className="gradient-icon" /> {t.replace('Extra ', '')}
            </button>
          ))}
          <div className="ew-right text-subtotal-orange" style={{ marginLeft: 'auto', fontWeight: 700 }}>Group subtotal: {fmt(groupSubtotal)}</div>
        </div>
      </AccordionSection>
    </div>
  );
