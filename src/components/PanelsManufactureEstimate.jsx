...
export default function PanelsManufactureEstimate({
  ...
  const [uiState, setUiState] = useLocalStorageJson(persistKey, {
    collapsed: false,
  });
  ...
  useEffect(() => {
    setRateByKey((prev) => ({
      exteriorWalls: rateTouched.exteriorWalls
        ? prev.exteriorWalls
        : rates.exteriorWalls?.ratePerLF ?? prev.exteriorWalls ?? 0,
      interiorShear: rateTouched.interiorShear
        ? prev.interiorShear
        : rates.interiorShear?.ratePerLF ?? prev.interiorShear ?? 0,
      interiorBlockingOnly: rateTouched.interiorBlockingOnly
        ? prev.interiorBlockingOnly
        : rates.interiorBlocking?.ratePerLF ?? prev.interiorBlockingOnly ?? 0,
      interiorNonLoad: rateTouched.interiorNonLoad
        ? prev.interiorNonLoad
        : rates.interiorNonLoad?.ratePerLF ?? prev.interiorNonLoad ?? 0,
      kneeWall: rateTouched.kneeWall
        ? prev.kneeWall
        : rates.kneeWall?.ratePerLF ?? prev.kneeWall ?? 0,
      windows: rateTouched.windows
        ? prev.windows
        : rates.windows?.each ?? prev.windows ?? 0,
      exteriorDoors: rateTouched.exteriorDoors
        ? prev.exteriorDoors
        : rates.exteriorDoors?.each ?? prev.exteriorDoors ?? 0,
      blocking2x10: rateTouched.blocking2x10
        ? prev.blocking2x10
        : rates.blocking2x10?.each ?? prev.blocking2x10 ?? 0,
    }));
  }, [rates, rateTouched]);
  ...
  useEffect(() => {
    setPanelLenByKey((prev) => ({
      ...
      interiorNonLoad: panelLenTouched.interiorNonLoad
        ? prev.interiorNonLoad
        : Number(panelLenFtInterior) || 8, // Use interior default
      kneeWall: panelLenTouched.kneeWall
        ? prev.kneeWall
        : Number(panelLenFtInterior) || 8, // Use interior default
    }));
  }, [
    ...
    panelLenFt,
    panelLenTouched,
  ]); // Removed duplicate panelLenFt

  // --- Manual LF/Qty State (for rows not driven by props) ---
  const manualInputKeys = useMemo(
    () => new Set(["windows", "exteriorDoors", "blocking2x10"]),
    []
  );
  const [manualInputByKey, setManualInputByKey] = useState(() => ({
    // Initialize directly
  ...
  useEffect(() => {
    setManualInputByKey((prev) => ({
      windows: manualInputTouched.windows
        ? prev.windows
        : safe.windows?.qty ?? 0,
      exteriorDoors: manualInputTouched.exteriorDoors
        ? prev.exteriorDoors
        : safe.exteriorDoors?.qty ?? 0,
      blocking2x10: manualInputTouched.blocking2x10
        ? prev.blocking2x10
        : safe.blocking2x10?.rows ?? 0,
    }));
  ...
  const lines = useMemo(() => {
    ...
    const pushLF = (label, key, rateConfigDefault) => {
      ...
      const ratePerLF = Number(
        rateByKey[key] ?? rateConfigDefault?.ratePerLF ?? 0
      ); // Use state value
    ...
    pushLF(
      "Interior wall with blocking only",
      "interiorBlockingOnly",
      rates.interiorBlocking
    );
    pushLF(
      "Interior Non-load bearing walls",
      "interiorNonLoad",
      rates.interiorNonLoad
    );
    ...
    panelLenByKey,
    rateByKey,
    manualInputByKey, // Use state values for calculations
    exteriorLF,
    interiorShearLF,
    interiorBlockingLF,
    interiorNonLoadLF,
    kneeWallLF, // Use props for LF values
  ]);
  ...
  const handlePanelLenChange = useCallback(
    (key) => (e) => {
    ...
    <AccordionSection
      ...
      bar={({ open, toggle }) => (
        ...
        >
          ...
          >
            Panel Manufacture subtotal: {money(totals.total)}
          </div>
        </div>
      )}
    >
