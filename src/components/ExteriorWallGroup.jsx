...
import ItemPicker from "./ItemPicker"; // Corrected import path
import { useLocalStorageJson } from "../hooks/useLocalStorageJson"; // Corrected import path
import {
  calcPlates,
  calcStuds,
  calcBlocking,
...
import AccordionSection from "./ui/AccordionSection"; // Corrected import path
import RemoveButton from "./ui/RemoveButton"; // Corrected import path
...
export default function ExteriorWallGroup({
  ...
  // Corrected getFamily function (was redefined)
  ...
  // PT LF in PANELS: PT bottom plates ≈ section length
  const ptLF = bottomIsPT ? Number(lengthLF || 0) : 0; // Only count LF if bottom is PT
  ...
  const addExtra = (type) =>
    setExtras((prev) => {
      ...
      let defaultWaste = 5; // Default waste for Header/Post
  ...
  const computedExtras = useMemo(() => {
    ...
    // Update dependencies if necessary (lengthLF, studSpacingIn, studMultiplier might be needed if used as fallbacks)
  ...
  useEffect(() => {
    const currentStats = {
      ...
      bottomPlatePiecesPanel: Math.ceil(rowByKey.bottomPlate?.qtyFinal || 0), // Added this
    ...
    panelSheets,
    platePieces,
    rowByKey.bottomPlate?.qtyFinal, // Dependency added
    ptLF,
    groupSubtotal,
    bottomBoardLenFt,
    panelPtBoards,
      ...
      >
        ...
        <div className="controls4" style={{ marginBottom: 12 }}>
          ...
          <label>
            ...
            <select
              className="ew-select focus-anim" // Use select class
              value={studMultiplier}
              onChange={(e) => setStudMultiplier(Number(e.target.value))} // Ensure value is stored as number
            >
              ...
              <Fragment key={ex.id}>
                <div className="ew-grid ew-row" style={{ "--cols": gridCols }}>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {ex.type !== "Headers infill" && (
                        <RemoveButton
                          onClick={() => removeExtra(ex.id)} // Corrected onClick
                  ...
                  <div>
                    <ItemPicker
                      compact
                      initialSelection={ex.item} // Pass initial selection if set in addExtra
                      ...
                      // Optionally try to default size based on main studs? More complex.
                      // defaultSizeLabel={ex.type === 'Stud' ? getSize(sel.studs) : undefined }
                    />
                      ...
                      >
                        ...
                        <label>
                          ...
                          <input
                            ...
                            type="number"
                            inputMode="decimal"
                            value={ex.inputs?.lengthLF ?? ""} // Use ?? '' for potentially undefined initial value
                            onChange={(e) =>
                              updateExtra(ex.id, {
                                inputs: {
                                  ...ex.inputs,
                        ...
                        <label>
                          ...
                          <select
                            ...
                            value={ex.inputs?.studMultiplier ?? 1} // Default select to 1 if undefined
                            onChange={(e) =>
                              updateExtra(ex.id, {
                                inputs: {
                                  ...ex.inputs,
