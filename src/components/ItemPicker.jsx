...
export default function ItemPicker({
  ...
  const setVendorIdIf = (val) =>
    setVendorId((prev) => (prev === toKey(val) ? prev : toKey(val)));
  const setFamilySlugIf = (val) =>
    setFamilySlug((prev) => (prev === toKey(val) ? prev : toKey(val)));
  const setSizeIdIf = (val) =>
    setSizeId((prev) => (prev === toKey(val) ? prev : toKey(val)));
  ...
  useEffect(() => {
    ...
    const byIncl = vendors.find((v) =>
      norm(v.displayName || "").includes(want)
    );
    ...
    (async () => {
      ...
      if (
        ...
        if (defaultFamilyLabel) {
          ...
          fam =
            fs.find((f) => norm(f.label) === want) ||
            fs.find((f) => norm(f.label).includes(want));
        }
        if (!fam) {
          ...
          fam =
            fs.find((f) => {
              ...
              return (
                n.includes("spf") && (n.includes("#2") || n.includes(" 2"))
              );
            }) || fs[0];
        }
        didAutoFamilyForVendor.current = vendorId;
  ...
  const vendorOptions = useMemo(
    () => vendors.map((v) => ({ value: String(v.id), label: v.displayName })),
    [vendors]
  );
  const familyOptions = useMemo(
    () => families.map((f) => ({ value: String(f.slug), label: f.label })),
    [families]
  );
  const sizeOptions = useMemo(
    () => sizes.map((s) => ({ value: String(s.id), label: s.sizeLabel })),
    [sizes]
  );
