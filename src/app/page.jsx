 ...
 export default function Home() {
    ...
    // Update the ref for the next render
     ...
     const content = useMemo(() => {
         if (authLoading) {
            return <div className="app-content"><div className="ew-card">Loading Authentication...</div></div>;
         }
         ...
         switch (active) {
            ...
            case 'wallpanels':
                // Can add a check here later if needed:
                // if (!currentProjectId) {
                //      return <div className="app-content"><div className="ew-card">Please select or create a project first.</div></div>;
                // }
                return (
                    ...
                    <div className="app-content">
                        <div className="ew-card">
                            <h2 className="ew-h2" style={{ marginTop: 0 }}>
                                {({ // Map keys to titles for placeholders
                                    auth:'Login / Logout',
                                    // project:'Project', // Handled by ProjectView
                                    trusses:'Trusses',
                                    loose:'Loose Material',
                                    labor:'Labor',
                                    // summary:'Summary', // Handled by Summary
    ...
    // Ensure all dependencies used inside useMemo (directly or indirectly) are listed
    }, [active, grandTotal, currentProjectId, user, authLoading]); // currentProjectId is used implicitly by WallPanelsView needing context
        ...
        <ProjectProvider initialProjectId={currentProjectId}>
            <div className={`app-shell ${collapsed ? 'sb-collapsed' : ''}`}>
                ...
                <main className="app-main">
                    {/* Content includes components that will consume the ProjectContext */}
                    {content}
                </main>
            </div>
        </ProjectProvider>
